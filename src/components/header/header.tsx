    ...
    return (
      <div className="fixed top-0 w-[100vw] h-[100vh] flex justify-center items-center bg-[#00000080] z-10">
        <div className="bg-white rounded-xl p-4 relative">
          <Form action={reg} className="flex flex-col gap-4 items-center">
            {isReg ? (
              <InputForm
                name="name"
                text="Имя"
            ...
            {isReg ? (
              <p
                className="flex font-Light text-black text-sm pb-2.5"
              ...
              <p className="flex font-Light text-Gray3 text-sm pb-2.5">
                У вас нет аккаунта? &nbsp;{" "}
                <span onClick={() => setIsReg(true)} className="text-black">
                  Зарегистрируйтесь.
                </span>
              </p>
            )}
            {error ? (
              <div className="font-Bold text-red-600 text-4xl">{error}</div>
            ) : (
              ""
            )}
            <input
      ...
      <header className="border-b border-Trans20 px-16 py-3 bg-white">
        <div className="flex justify-between items-center container mx-auto">
          ...
          <nav className="flex gap-16">
            {links.map((elem, i) => {
              return <LinkCustomized text={elem.text} url={elem.url} key={i} />;
            })}
          </nav>
          <div
            onClick={
              session.status == "authenticated" ? onLogOutHandler : openModal
            }
          >
            <Button
              text={session.status == "authenticated" ? "Выйти" : "Войти"}
              font="Bold"
              isBlue={true}
              link=""
              hoverArrow=""
