    ...
    return (
        <Link href={link}>
            <button
                ...
                type={buttontype}
                className={
                    "px-10 flex items-center transition duration-150 rounded-md border-Blue border py-2.5 font-" +
                    font +
                    (isBlue
                        ? " bg-Blue  text-white  hover:border hover:bg-BgWhite hover:text-black"
                        : "  bg-BgWhite text-black hover:bg-Blue hover:text-white") +
                    (arrow ? " gap-2" : " gap-0")
                }
            >
                {text}
...
export function CustomH({ text, star }: { text: string; star: string }) {
    return (
        <p className="font-Black text-Black text-6xl pb-10 relative text-center">
            {text}
            <img
                className="absolute bottom-28  inset-x-2/4"
                src={star}
                alt=""
            />
        </p>
